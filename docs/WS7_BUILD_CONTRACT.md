...
| **T11** RBAC leaks cost/margin | — | Field-level, not screen-level. A "restricted" role must not leak margin. | ✅ P1-7: `landing.mask_field` at the /api serialization boundary (per-employee salary → salary_detail; margin/unit-economics → confidential); masked value ABSENT from the body (byte-level-tested), SPA renders an explicit lock chip with the reason |
