...
export function payslipPdf(data: PayslipData): Promise<Buffer> {
  const doc = new PDFDocument({ size: 'A4', margin: 48, info: { Title: `Payslip ${data.period}`, Author: 'Maisha-Mahsa' } });
  doc.fontSize(18).text(`${data.company} — Payslip`);
  doc.moveDown(0.3).fontSize(10).fillColor('#444').text(`${data.employee_name} (${data.employee_code})  ·  ${data.period}`);
  doc.moveDown(1).fillColor('#000');

  const rows = Math.max(data.earnings.length, data.deductions.length);
  doc.fontSize(11).text('Earnings', 48, doc.y, { continued: false });
  const startY = doc.y;
  ...
  doc.fontSize(10.5).text('Gross', 48, totalsY).text(inr(data.gross), 200, totalsY, { width: 90, align: 'right' });
  doc.text('Total deductions', 320, totalsY).text(inr(data.total_deductions), 470, totalsY, { width: 90, align: 'right' });
  doc.moveDown(3).fontSize(14).text(`Net pay: ${inr(data.net)}`, 48);
  return toBuffer(doc);
}
...
export function form16Pdf(data: Form16Data): Promise<Buffer> {
  const doc = new PDFDocument({ size: 'A4', margin: 48, info: { Title: `Form 16 ${data.financial_year}`, Author: 'Maisha-Mahsa' } });
  doc.fontSize(16).text('FORM NO. 16 — Part B');
  doc.moveDown(0.3).fontSize(9).fillColor('#444').text('[See rule 31(1)(a)] · Certificate under section 203 of the Income-tax Act, 1961');
  ...
  allRows.forEach(([label, amount], i) => {
    const y = startY + i * 18;
    doc.text(label, 48, y).text(inr(amount), 400, y, { width: 110, align: 'right' });
  });
  doc.moveDown(allRows.length + 1);
