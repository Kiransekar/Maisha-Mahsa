...
def test_claims_round_trip_for_plain_194c_and_194h(session):
    ...
    svc.create_bill(
        session, bill_number="B1", vendor_id=vc.id, bill_date="2026-05-10",
        subtotal=Paise.from_rupees(40000),
    )
    vh = _vendor(session, tds_section="194H")
    svc.create_bill(
        session, bill_number="B2", vendor_id=vh.id, bill_date="2026-05-11",
...
def test_claims_recover_194j_technical_category(session):
    ...
    res = svc.create_bill(
        session, bill_number="B3", vendor_id=v.id, bill_date="2026-05-12",
        subtotal=Paise.from_rupees(100000), tds_category="technical",
    )
...
def test_claims_recover_194i_plant_category(session):
    ...
    res = svc.create_bill(
        session, bill_number="B4", vendor_id=v.id, bill_date="2026-05-13",
        subtotal=Paise.from_rupees(60000), tds_category="plant",
    )
...
def test_no_claim_below_threshold_or_without_tds_section(session):
    ...
    svc.create_bill(
        session, bill_number="B5", vendor_id=below.id, bill_date="2026-05-14",
        subtotal=Paise.from_rupees(1000),  # below ₹50k threshold -> tds_amount 0
    )
    no_section = _vendor(session, tds_section=None)
    svc.create_bill(
        session, bill_number="B6", vendor_id=no_section.id, bill_date="2026-05-15",
...
def test_only_tds_on_payment_targets_emitted(session):
    ...
    svc.create_bill(
        session, bill_number="B7", vendor_id=v.id, bill_date="2026-05-16",
        subtotal=Paise.from_rupees(40000),
    )
    claims = svc.recompute_claims(session)
