...
def _seed(session):  # type: ignore[no-untyped-def]
    ...
    session.add(
        BankTransaction(
            account_id=acct.id, txn_date="2026-05-21", debit=Paise.from_rupees(1000), credit=0
        )
    )  # uncategorised
    session.add(
        BankTransaction(
            account_id=acct.id, txn_date="2026-05-22", debit=0, credit=Paise.from_rupees(7000)
        )
    )  # credit -> ignored
    session.flush()
