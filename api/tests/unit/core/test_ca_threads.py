...
def test_full_lifecycle_chains_every_event_into_the_verifiable_audit_log(session):
    ...
    respond_thread(
        session, thread_id=thread.id, timestamp=TS, note="invoice attached",
        doc_id=doc.id, user_id="acct-1",
    )
    assert thread.state == "responded"

    resolve_thread(
        session, thread_id=thread.id, timestamp=TS, note="satisfied", user_id="ca-1"
    )
    assert thread.state == "resolved"

    chain = load_chain(session)
    assert [e.action for e in chain] == [
        "ca_thread.raise", "ca_thread.respond", "ca_thread.resolve"
    ]
...
def test_transitions_and_doc_link_are_enforced_and_refusals_seal_nothing(session):
    doc = _doc(session)
    thread = raise_thread(
        session, timestamp=TS, domain="gst", entry_ref="journal:7",
        question="Support for this ITC claim?", user_id="ca-1",
    )
    ...
    with pytest.raises(LookupError, match="unknown vault document"):
        respond_thread(
            session, thread_id=thread.id, timestamp=TS, note="x",
            doc_id="f" * 64, user_id="acct-1",
        )
    ...
    respond_thread(
        session, thread_id=thread.id, timestamp=TS, note="here", doc_id=doc.id,
        user_id="acct-1",
    )
    resolve_thread(session, thread_id=thread.id, timestamp=TS, note="", user_id="ca-1")
    with pytest.raises(ValueError, match="resolved"):
        respond_thread(
            session, thread_id=thread.id, timestamp=TS, note="late", doc_id=doc.id,
            user_id="acct-1",
        )
...
def test_raise_rejects_empty_pin_or_question(session):
    with pytest.raises(ValueError):
        raise_thread(
            session, timestamp=TS, domain="ledger", entry_ref="  ", question="q",
            user_id="ca-1",
        )
    with pytest.raises(ValueError):
        raise_thread(
            session, timestamp=TS, domain="ledger", entry_ref="journal:1", question=" ",
            user_id="ca-1",
        )
...
def test_sample_is_deterministic_and_seeded_by_org_and_spec(session):
    ...
    assert {v["voucher_id"] for v in s_other["sample"]} != {
        v["voucher_id"] for v in s1["sample"]
    }

    # Spec is part of the seed: same n, shifted window, different draw order.
    filters_hold = sample_selection(
        session, org="org-7", domain="payroll", date_from="2026-07-01",
        date_to="2026-07-31", n=5,
    )
    ...
    windowed = sample_selection(
        session, org="org-7", domain="gst", date_from="2026-07-01",
        date_to="2026-07-04", n=10,
    )
...
def test_sample_rejects_nonpositive_n(session):
    with pytest.raises(ValueError):
        sample_selection(
            session, org="org-7", domain=None, date_from="2026-07-01",
            date_to="2026-07-31", n=0,
        )
