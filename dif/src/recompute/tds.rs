...
/// TDS on a single payment of `amount` paise (taxable value). `payee_type` selects the 194C
/// rate (1% individual/HUF vs 2% other); `category` selects the sub-rate (e.g. "technical" for
/// 194J, "plant" for 194I); `aggregate_ytd` is the running annual total.
...
mod tests {
    ...
    fn tds_194j_below_new_threshold() {
        ...
        let r = tds_on_payment("194J", 4_000_000, "company", None, 0);
        assert!(!r.applicable);
        assert_eq!(r.tds_paise, Paise(0));
    }
    ...
    fn tds_194i_40k_no_tds() {
        ...
        let r = tds_on_payment("194I", 4_000_000, "company", Some("building"), 12_000_000);
        assert!(!r.applicable);
        assert_eq!(r.tds_paise, Paise(0));
    }
    ...
    fn tds_194i_55k_full_month() {
        ...
        let r = tds_on_payment("194I", 5_500_000, "company", Some("building"), 0);
        assert!(r.applicable);
    ...
    fn t_194j_below_threshold_no_tds() {
        let r = tds_on_payment("194J", 2_000_000, "company", None, 0);
        assert!(!r.applicable);
        assert_eq!(r.tds_paise, Paise(0));
    }
    ...
    fn t_194c_rate_depends_on_payee_type() {
        // 194C: 2% for a company payee, 1% for individual/HUF — recomputed through the public fn.
        let company = tds_on_payment("194C", 4_000_000, "company", None, 0);
        assert_eq!(company.tds_paise, Paise(80_000)); // 2% of ₹40k = ₹800
        let individual = tds_on_payment("194C", 4_000_000, "individual", None, 0);
        assert_eq!(individual.tds_paise, Paise(40_000)); // 1% of ₹40k = ₹400
        let huf = tds_on_payment("194C", 4_000_000, "huf", None, 0);
        assert_eq!(huf.tds_paise, Paise(40_000));
    }

    #[test]
    fn t_194c_aggregate_threshold_triggers_tds() {
        // single ₹20k < ₹30k, but YTD ₹90k + ₹20k = ₹1.1L >= ₹1L aggregate -> TDS applies.
        let r = tds_on_payment("194C", 2_000_000, "company", None, 9_000_000);
        assert!(r.applicable);
    ...
    fn t_fractional_paise_single_round_matches_python() {
        ...
        let r = tds_on_payment("194I", 5_000_495, "company", Some("building"), 0);
        assert!(r.applicable);
        assert_eq!(r.tds_paise, Paise(500_000));
    }
    ...
    fn t_194j_technical_rate() {
        ...
        let r = tds_on_payment("194J", 10_000_000, "company", Some("technical"), 0);
        assert_eq!(r.tds_paise, Paise(200_000));
    }

    #[test]
    fn t_unknown_section_not_applicable() {
        let r = tds_on_payment("999X", 10_000_000, "company", None, 0);
        assert!(!r.applicable);
        assert_eq!(r.tds_paise, Paise(0));
    }
}
